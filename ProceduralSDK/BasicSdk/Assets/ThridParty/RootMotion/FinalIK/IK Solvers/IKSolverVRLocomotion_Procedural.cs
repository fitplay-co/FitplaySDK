...
using VRIK;
using VRIK.State;
using static RootMotion.FinalIK.FitPlayProcedural.WalkStateEnum;
...
namespace RootMotion.FinalIK
{
    public partial class IKSolverVR : IKSolver
    {
        public partial class Locomotion
        {
            ...
            private LocomotionStateMachine<LocomotionFsmComponent> machine;
            private ProceduralStartWalkingState start;
            private ProceduralLoopWalkingState loop;
            private ProceduralWalkCatchUpState catchup;
            ...
            private void Initiate_Procedural(Vector3[] positions, Quaternion[] rotations, bool hasToes, float scale)
            {
                ...
                OSDataInput osInput = new OSDataInput();
                CalculatorModule calc = new CalculatorModule(osInput);
                
                ProceduralSetup setup = new ProceduralSetup();
                StateSharedInfo shared = new StateSharedInfo();
                
                setup.stepHeight = stepHeight;
                setup.heelHeight = heelHeight;
                
                
                LocomotionFsmComponent comp = new LocomotionFsmComponent();
                comp.OnInit();
                
                start = (ProceduralStartWalkingState) comp.States[WalkStart];
                loop = (ProceduralLoopWalkingState) comp.States[WalkLoop];
                catchup = (ProceduralWalkCatchUpState) comp.States[WalkCatchup];


                shared.SetLeftFootStep(rotations[0], positions[leftFootIndex], rotations[leftFootIndex], footDistance * scale * Vector3.left);
                shared.SetRightFootStep(rotations[0], positions[rightFootIndex], rotations[rightFootIndex], footDistance * scale * Vector3.right);
                
           
                InitOnce(start, setup, calc,shared, WalkStart);
                InitOnce(loop, setup, calc,shared, WalkLoop);
                InitOnce(catchup, setup, calc,shared, WalkCatchup);
            ...
            public static void InitOnce(ProceduralState state, ProceduralSetup setup, CalculatorModule calc,
                StateSharedInfo stateSharedInfo,
                WalkStateEnum stateEnum)
            {
                
                state.SetProceduralState(calc,setup,stateSharedInfo);
                state.SetEnum(stateEnum);
            }

            public void Solve_Procedural_New(BaseAvatarIKInfo basicInfo)
            {
                start.SetDataGroup(basicInfo);
                loop.SetDataGroup(basicInfo);
                catchup.SetDataGroup(basicInfo);
                machine.OnUpdate(Time.deltaTime);
