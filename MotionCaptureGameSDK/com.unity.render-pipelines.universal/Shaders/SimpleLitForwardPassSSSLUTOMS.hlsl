...
//#ifdef _NORMALMAP
    ...
    //变大减弱
    //output.posWS.xyz += output.normal * _Roughness;
    // //前进类似
    //output.posWS.xyz += viewDirWS * _Roughness;
    //后退被覆盖
    //output.posWS.xyz -= viewDirWS * _Roughness;
    //Light mainLight = GetMainLight(inputData.shadowCoord, inputData.positionWS);
    //float3 lightDir = mainLight.direction;
    //output.posWS.xyz += _MainLightPosition.xyz * _Roughness;
...
half4 LitPassFragmentSimpleSSSLUT(Varyings input) : SV_Target
{
    ...
    //finalColor = (1 - (mainLight.shadowAttenuation));
    finalColor = lerp(_Alpha0, _Alpha1, (mainLight.shadowAttenuation));
    //finalColor = lerp(_Alpha0, _Alpha1, _SpecColor.rgb * (mainLight.shadowAttenuation));
    //finalColor = lerp(0.3, 1, (mainLight.shadowAttenuation));
