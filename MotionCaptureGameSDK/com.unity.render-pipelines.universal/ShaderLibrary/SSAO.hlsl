...
//Texture2D<float> _SSRT_CurveRT;
TEXTURE2D_FLOAT(_SSRT_CurveRT);
SAMPLER(sampler_SSRT_CurveRT);
float _IntensityVar;
...
float4 SSAO(Varyings input) : SV_Target
{
    ...
    ao *= RADIUS;
    float fCurveValue = _IntensityVar*SAMPLE_TEXTURE2D(_SSRT_CurveRT, sampler_SSRT_CurveRT, float2(depth_o,0));
    // Apply contrast
    ao = PositivePow(ao * (INTENSITY + fCurveValue )* rcpSampleCount, kContrast);
