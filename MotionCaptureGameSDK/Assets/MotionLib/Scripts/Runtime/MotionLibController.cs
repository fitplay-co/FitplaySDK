...
namespace MotionLib.Scripts
{
    public class MotionLibController : MonoBehaviour
    {
        ...
        public float ElbowAngleMin = 70;
        [Header("ElbowAngleMin < 两侧肘关节夹角 < ElbowAngleMax 时，第四触发条件成立")] [SerializeField] [Range(0, 90)]
        public float ElbowAngleMax = 90;
        

        [FormerlySerializedAs("HowToTurn")] [SerializeField] public MotionMode howToMotion = MotionMode.Motion5;
        ...
        bool IsTraveMode()
        {
            return (standTravelModelManager != null && 
                    standTravelModelManager.currentMode == StandTravelModel.Scripts.Runtime.MotionMode.Travel);//standTravelModelManager.Enabled &&
        }
        

        private bool WristToShoulderXCorrect(Vector3 leftElbow, Vector3 rightElbow, Vector3 leftShoulder, Vector3 rightShoulder)
        {
            if(Math.Abs(leftShoulder.x - leftElbow.x) > WristToShoulderX)
                return false;
            if(Math.Abs(rightShoulder.x - rightElbow.x) > WristToShoulderX)
                return false;
            return true;
        }

        private bool ElbowsAngleCorrect(Vector3 leftHand, Vector3 rightHand,Vector3 leftElbow, Vector3 rightElbow, Vector3 leftShoulder, Vector3 rightShoulder)
        {
            float angleL = Vector3.Angle(leftHand, leftShoulder);
            if (angleL > ElbowAngleMin && angleL < ElbowAngleMax)
            {
                float angleR = Vector3.Angle(rightHand, rightShoulder);
        ...
        void FixedUpdate()
        {
            if (IsTraveMode())
            {
                ...
                var motionType1 = WristToShoulderYCorrect(leftHand, leftShoulder, leftHip, true);
                var motionType2 = WristToShoulderYCorrect(rightHand, rightShoulder, rightHip, false);
