...
namespace UnityEditor.Rendering.LWRP.ShaderGUI
{
    public static class ScheuermannAnistropyGUI
    {
        ...
        public static void DoSmoothness(SimpleLitProperties properties, Material material)
        {
            ...
            float anistropyScale = 0.0f;
            if (properties.anistropyScale != null)
            { 
                anistropyScale = properties.anistropyScale.floatValue;
            ...
            float LambertRemap0 = 0.0f;
            float LambertRemap1 = 0.0f;
            if (properties.LambertRemap0 != null)
            {
                LambertRemap0 = properties.LambertRemap0.floatValue;
                LambertRemap0 = EditorGUILayout.Slider(Styles.LambertRemap0Text, LambertRemap0, -1f, 2f);
                LambertRemap1 = properties.LambertRemap1.floatValue;
            ...
            //美术要求取消取整
            float vH2 = (EditorGUILayout.Slider(Styles.HueText, (vH * 255f), 0f, 255f) / 255f);
            float vS2 = (EditorGUILayout.Slider(Styles.SaturationText, (vS * 255f), 0f, 255f) / 255f);
            float vV2 = (EditorGUILayout.Slider(Styles.ValueText, (vV * 255f), 0f, 255f) / 255f);
            if (EditorGUI.EndChangeCheck())
            {
