...
half3 LightingPhysicallyBasedFabric(BRDFData brdfData, BRDFData brdfDataClearCoat, Light light, half3 normalWS, half3 viewDirectionWS, half clearCoatMask, bool specularHighlightsOff, half3 tangentWS, half3 bitangentWS)
{
    return LightingPhysicallyBasedFabric(brdfData, brdfDataClearCoat, light.color, light.direction, light.distanceAttenuation * light.shadowAttenuation, normalWS, viewDirectionWS, clearCoatMask, specularHighlightsOff, tangentWS, bitangentWS);
}
...
//    return light;
//}
half4 UniversalFragmentFabricPBR(InputData inputData, SurfaceData surfaceData, half3 tangentWS, half3 bitangentWS)
{
...
//half4 maintextAlbedo = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv);
    float maintextAlbedo = SampleMT(input.uv);
...
#endif
    half4 color = UniversalFragmentFabricPBR(inputData, surfaceData, input.tangentWS.rgb,bitangent);
