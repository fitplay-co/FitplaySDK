...
half4 LitPassFragmentSimpleSSSLUT(Varyings input) : SV_Target
{
    ...
    diffuseSSS = SAMPLE_TEXTURE2D(_SSSLUT, sampler_SSSLUT, float2(NdotL * 0.5 + 0.5, alpha)).xyz;
    ...
    //finalColor = albedo * (1 - SafeNormalize(diffuseColor));
    //finalColor = lerp(finalColor, specular3, _Roughness);
    ...
    // 0.335
    //finalColor = step(_Roughness, SafeNormalize((diffuseColor)).b) * (1 - SafeNormalize((diffuseColor)));
    //finalColor = (1 - SafeNormalize((diffuseColor)));
    //finalColor = albedo * diffuseSSS * (1 - SafeNormalize((diffuseColor)));
    //finalColor = albedo * diffuseSSS * (1 - SafeNormalize((mainLight.shadowAttenuation)));
